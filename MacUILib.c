...
print_ptr MacUILib_printf;
	...
	{
		usleep(usec); // used for introducing delays
	}
	
	int MacUILib_hasChar(void)
	{
		return _kbhit(); // allows program to continue running while checking for input
	}
	
	char MacUILib_getChar(void)
	{
		return _getch(); // returns ASCII value of the key that was pressed, useful for determining which key was pressed
	}
...
#ifdef POSIX //Portable operating system interface
